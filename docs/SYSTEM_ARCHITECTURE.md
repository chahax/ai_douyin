...
flowchart LR
    ...
    E --> F["compose_video"]
    F --> G["final mp4"]
    G --> H["DouyinAdapter publish"]
```

关键点：

- `compose_video()` 使用 FFmpeg `stream_loop` 把模板视频循环到配音长度。
- 输出 mp4 由 `douyin-publish` 或 `auto-publish` 发布。
- 这是当前唯一接入一键发布的生成视频模式。
...
```text
AutoPublishService
  -> mode=single      单人口播模板视频
  -> mode=dual        双角色视频
  -> mode=framepack   FramePack 后处理视频
```
