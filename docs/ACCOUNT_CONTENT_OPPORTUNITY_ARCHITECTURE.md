...
- 发布归因上下文已扩展到账号/策略版本、机会、脚本、A/B 版本、钩子、展示方式、
  节奏、工作流和发布时间窗；抖音作品同步会追加真实指标快照；
- 支持 1h、6h、24h、72h、7d 时间窗缺口检测和同账号、同时长、同时间窗基线；
- 复盘输出话题、机会、脚本版本、钩子、展示方式、工作流、发布时间窗的胜率、
  相对表现和置信度，并保存版本化学习报告；
- 下一轮机会评分读取最近学习报告，将已验证维度作为反馈先验，同时保留
  70% 已验证、20% 相邻、10% 新实验的探索配额。

主功能阶段已完成；剩余工作是全量迁移演练、主页面接线、工作流节点兼容、
静态检查、完整测试集和端到端烟测。
...
实现说明：累计播放不直接跨账号比较；相对表现使用同账号、同时长桶、同指标
时间窗的播放增速中位数作为基线。任何维度样本越少，学习报告越向中性 50 分
收缩；少于 3 条有效视频时只观察、不改变稳定策略。学习报告不会自动覆盖生产
账号配置，而是在下一轮机会分中作为有证据的先验项。
