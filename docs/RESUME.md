...
- **AI Douyin — 基于 LLM Agent 的本地化短视频内容生产与抖音运营自动化平台**
- **AI Douyin — 对话式 AI 短视频生成 + 抖音自动化运营系统**
- **AI Douyin — 从脚本到发布的端到端 AI 短视频工厂**

---

## 一句话定位

> 用自然语言一句话驱动整个短视频生产 + 抖音运营流程：LLM 生成脚本 → Edge-TTS/GPT-SoVITS 配音 → Sonic + ComfyUI 数字人视频合成 → 浏览器自动化发布 → 抓评论与自动回复 → 任务定时调度，全链路本地化运行。
...
> **AI Douyin** 是本地运行的对话式 AI 短视频生产与抖音运营平台。我独立设计并实现了：(1) 可由 LLM 自动调度的 Skill Registry（18+ 个能力，含生成/发布/评论/养号/番茄推广）；(2) "先生成计划、用户确认、再执行"的安全 Agent；(3) APScheduler + SQLite 任务队列的后台调度；(4) 动漫数字人主讲视频的端到端管线（Edge-TTS + Sonic + ComfyUI + FFmpeg）；(5) Selenium 驱动的抖音浏览器自动化（发布/同步/评论/回复）。Streamlit 管理后台覆盖视频、评论、对话、调度等所有运营场景。

---

## 项目描述（中等 · 4-6 行）

> **AI Douyin** 是我从零搭建的端到端 AI 短视频内容平台，核心解决"个人/小团队如何用 AI 自动化抖音运营"的问题。
>
> 我独立负责整体架构与实现：
>
...
> - **抖音浏览器自动化**（`src/platform_adapter/`）：Selenium 持久登录态、发布、同步视频、抓评论、规则/LLM 自动回复、多账号养号（精选页推荐/可控点赞/评论区浏览）；番茄小说推广 MVP 复用同套浏览器会话。
...
> **AI Douyin** 是我从零搭建、本地化运行的对话式 AI 短视频内容生成与抖音运营自动化平台，覆盖从脚本生成、视频合成到抖音发布、评论回复、账号养号的完整闭环。
>
> ### 架构与亮点
>
> **1. LLM Agent + Tool-Use 调度**
>
> - 把全平台能力（内容生成 / 平台运营 / 番茄推广 / 养号 / 知识库 / 系统工具）封装为 18+ 个 Skill 注册到统一 Registry；LLM 通过 prompt 自动选择并调用。
...
> - 番茄小说推广 MVP：复用浏览器会话，登录态 / 申请推广 / 获取章节 / 生成推广视频全链路跑通。
>
...
> - **多 Skill 协作**：所有平台能力（生成 / 发布 / 评论 / 养号 / 番茄）已注册到 Skill Registry，可由 LLM 一句话串起来。
...
- 独立设计并实现本地化 LLM Agent + Skill Registry：18+ 个 Skill（生成 / 发布 / 评论 / 养号 / 番茄推广 / 知识库 / 记忆管理）由 LLM 通过 prompt 自动调度，写操作 100% 走"先生成计划、用户确认、再执行"安全流程。
...
- 实现 Selenium 驱动的抖音浏览器自动化：持久登录态、发布、同步创作者后台、抓评论、规则/LLM 自动回复（限流 + 违禁词 + 历史），覆盖多账号养号（精选页推荐入口 / 可控点赞 / 评论区浏览）。
- 搭建番茄小说推广 MVP：复用浏览器会话支持登录态 / 申请推广 / 获取章节 / 生成 Presenter 推广视频。
...
- 平台覆盖：抖音（发布 / 同步 / 评论 / 回复 / 养号）+ 番茄（推广申请 / 章节获取 / 推广视频）。
