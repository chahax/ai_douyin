...
1. [当前能力总览](CURRENT_CAPABILITIES.md)：现在已经能做什么，尤其是视频生成。
...
6. [关联项目与视频生成集成说明](RELATED_PROJECTS_INTEGRATION.md)：FramePack、FramePack_oneclick、背景素材与当前项目的关系。
7. [账号登录与用量控制方案](ACCOUNT_LOGIN_AND_USAGE_PLAN.md)：邮箱验证码登录、角色权限和每用户额度设计。
8. [小说推广视频平台支线设计](NOVEL_PROMOTION_VIDEO_PLATFORM_DESIGN.md)：番茄推广平台、小说前 10 章提炼、推广视频生成和绑定规划。
9. [Cloudflared 公网访问](CLOUDFLARED_PUBLIC_ACCESS.md)：用 Cloudflare Quick Tunnel 临时开放管理后台公网访问。
10. [docs 文档保留与遗弃分析](DOCS_RETENTION_ANALYSIS_2026-05-20.md)：判断历史文档是否可遗弃。
11. [Codex 会话导入摘要](CODEX_SESSION_IMPORT_2026.md)：从 Codex 2026 原始记录整理出的历史决策和排查结论。

视频生成相关主线：

- [动漫数字人主讲视频优化方案](ANIME_DIGITAL_HUMAN_PLAN.md)
- [Presenter 输入通道改造设计](PRESENTER_INPUT_CHANNELS_DESIGN.md)
- [背景图图片质检与重抽设计](BACKGROUND_IMAGE_REVIEW_DESIGN.md)
