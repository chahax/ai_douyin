...
- ⏳ D0.6 待下：PuLID 权重 (~1.5GB) + AntelopeV2 (~300MB) → 用户后台跑

| 编号 | 名称 | 工作量 | 依赖 | 状态 |
|---|---|---|---|---|
| **D0.6** | 下 PuLID + AntelopeV2 权重 | 0.5h | — | ⏳ 用户后台跑 |
| **V5-1** | L1 LLM 拆镜 + Pydantic schema（不依赖 D0.6）| 1d | I-3 | ⏳ 待开始 |
| **V5-2** | Wan 2.2 ComfyUI workflow（依赖 D0.1）| 1d | D0.1 ✅ | ⏳ 待开始 |
| **V5-3** | keyframe_gen_xl workflow（依赖 D0.2 + D0.6）| 1d | D0.6 | ⏳ 待开始 |
...
**关键依赖**：
- V5-1 (Pydantic schema) 不依赖 PuLID → 可立即开
- V5-2 (Wan workflow) 依赖 WanVideoWrapper → 节点已装，可立即开
- V5-3 (keyframe workflow) 依赖 PuLID 权重 → 等 D0.6
