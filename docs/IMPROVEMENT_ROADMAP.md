...
> 输入小说 → LLM 拆镜 → 每段生成 AI 视频 → 拼接成片。模型权重**已就位**，无需下载。

| 编号 | 名称 | 工作量 | 依赖 | 状态 |
|---|---|---|---|---|
| **V5-1** | L1 LLM 拆镜 + Pydantic schema（scene_plan / dialogue / novel_split）| 1d | I-3 | ⏳ 待开始 |
| **V5-2** | Wan 2.2 ComfyUI workflow JSON（含 4 步加速 LoRA）| 1d | — | ⏳ 待开始 |
| **V5-3** | wan_pipeline.py + I-2 容错复用（OOM 重试 + 降级到静态图）| 2d | V5-2 | ⏳ 待开始 |
| **V5-4** | video_concat.py（拼视频 + narration 音频 + BGM + 字幕 + 段间 LPIPS 校验 + 音视频对齐）| 1d | V5-3 | ⏳ 待开始 |
| **V5-5** | v5_pipeline.py 端到端 + CLI `novel-video` + 单测 | 2d | V5-4 | ⏳ 待开始 |

合计 7-8 天。模型权重**全部就位**（[D:/IT/AI_vido/ComfyUI/models/](D:/IT/AI_vido/ComfyUI/models/)）：
