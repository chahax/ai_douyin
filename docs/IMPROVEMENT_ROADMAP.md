...
### Phase 2.5 · V5 小说→纯视频 Pipeline（NEW，基于 docs/design/v5-pure-video-pipeline.md）

> V4 用"数字人 + 静态背景图"，V5 切换为"无角色 + AI 视频片段"（Wan 2.2 i2v）。
> 输入小说 → LLM 拆镜 → 每段生成 AI 视频 → 拼接成片。

**D0 setup 状态**（✅ 全部就绪）：
- ✅ `ComfyUI-WanVideoWrapper` 已装
- ✅ `PuLID_ComfyUI` 已装
- ✅ Wan 2.2 模型全套完整（diffusion/vae/LoRA/text_encoder 共 17GB）
- ✅ `ip-adapter_pulid_sdxl_fp16.safetensors` (755MB) — 用户手动下（hf-mirror）
- ✅ AntelopeV2 6 个文件 (408MB) — 用户手动下（gh-proxy），解压到正确子目录
- ⏳ EVA02-CLIP-L-14-336 — 首次运行 PuLID 节点时自动下到 HF cache

| 编号 | 名称 | 工作量 | 依赖 | 状态 |
|---|---|---|---|---|
| **D0.6** | 下 PuLID + AntelopeV2 权重 | 0.5h | — | ✅ **已完成**（用户手动下） |
| **V5-1** | L1 LLM 拆镜 + Pydantic schema | 1d | I-3 | ✅ **已完成**（d8cec68）|
| **V5-2** | Wan 2.2 ComfyUI workflow | 1d | D0.1 ✅ | ⏳ 待开始 |
| **V5-3** | keyframe_gen_xl workflow | 1d | D0.6 ✅ | ⏳ 待开始 |
| **V5-4** | wan_pipeline.py + I-2 容错复用 | 2d | V5-2 | ⏳ 待开始 |
| **V5-5** | video_concat.py（拼 + 段间 LPIPS + 音视频对齐）| 1d | V5-4 | ⏳ 待开始 |
| **V5-6** | v5_pipeline.py 端到端 + CLI `novel-video` + 单测 | 2d | V5-5 | ⏳ 待开始 |

合计 7-8 天。

复用 ship.md 附录 A：
- A.4.1 L0 schema 校验（V5-1）
- A.4.2 L1 业务校验（段时长 3-5s、角色白名单、段数比例）
- A.5.2 retry 参数变体（V5-1）
- A.6 二审（Qwen-as-judge）— 长期，先不做
- A.7 execute_task 状态机 — 长期，先不做

**关键依赖**：
- V5-1 (Pydantic schema) ✅ 已完成
- V5-2 (Wan workflow) 依赖 WanVideoWrapper ✅ 节点已装，可立即开
- V5-3 (keyframe workflow) ✅ 权重已到位，可立即开

设计文档（[docs/design/v5-pure-video-pipeline.md](v5-pure-video-pipeline.md)）包含的额外细节：
- **节点真名**（非内置名）：`WanVideoModelLoader` / `WanVideoImageToVideoEncode` / `WanVideoSampler` / `WanVideoDecode`（来自 `kijai/ComfyUI-WanVideoWrapper`）
- **路径修正**：`umt5_xxl_fp8_e4m3fn_scaled.safetensors` 在 `models/text_encoders/`，**不是** `models/clip/`
- **2D 风格选择**：DreamShaper XL（默认）/ AnythingXL（备选）/ RealVisXL（写实慎用）
- **首末帧生成工作流**（独立 `keyframe_gen_xl.json`）：Checkpoint + PuLID（角色一致性 0.85）
- **段间连贯性校验**：concat 前 LPIPS < 0.3 自动校验，超阈值 quarantined
- **音视频对齐策略**：音频优先切割视频（对白类内容优先）
- **动态 max_segments**：基于字数 4-15 镜（每镜 250 字）
- **D2 / D3 验证节点**：实测数据采集，让预估变成事实
...
| 2026-06-28 | **V5-1 L1 拆镜 + D0.6 权重完成** | novel_schemas.py (Pydantic: DialogueLine / ScenePlan / NovelSplit) + novel_splitter.py (3-prompt retry 变体) + 67 个新单测全过。修复 normalize_json_content 剥 reasoning model <think> 块（DeepSeek-R1 / MiniMax-M2 / QwQ）。PuLID (754.7MB) + AntelopeV2 (407.7MB) 用户手动下完。 |
...
| v5 | 2026-06-28 | V5-1 L1 拆镜 + D0.6 权重完成 + 11 个改进并入设计文档（首末帧模型 / 段间连贯性 / 音视频对齐 / 风格选择 / 动态 max_segments / D2-D3 验证节点 / PuLID WanVideoWrapper 节点真名 / umt5 路径修正）|