...
针对首版出现的额外手、曝光漂移、长对白末帧静止、人物身份漂移和口型不统一问题，V2 改为每位角色只使用一张固定人物母版，并用完整对白音频驱动 SadTalker 头像动画。场景信息保留在强虚化背景中，清晰主体不再交给视频扩散模型重绘。

- 林小雨与陈默各自全片复用唯一身份锚点。
- 所有对白镜头均由音频驱动口型；不再在提示词中要求随机嘴部微动。
- Edge-TTS 支持逐镜头 `rate`、`volume`、`pitch`，用于区分温柔、犹豫、兴奋、逼迫和醒悟阶段。
- 镜头 02 不再显示清晰手部道具；镜头 10 不再发生曝光漂移；镜头 11 生成与对白等长的 233 帧素材，未检测到 1.5 秒以上冻结。
- 最终交付：`final/perfect_lover_v2_lipsync_captioned.mp4`，1080×1920、30 fps、175.507 秒。

## V3 可控手部与手机动作修复

V3 不再使用全程头像方案。对已核验安全的原始动作，只保留开头 0.9–1.6 秒作为动作插入，然后在同一条音频时间线上切入固定人物母版驱动的口型画面。

- 保留双手持杯、双手持手机、单手拿手机和另一只手轻微示意等可读动作。
- 清晰手机区域只显示背面或侧面，不让生成模型承担屏幕文字和界面。
- 镜头 02 在第三只手出现前的 0.9 秒截断；异常帧未进入成片。
- 镜头 10、11 不复用扩散视频，分别规避曝光漂移和长时间静止。
- 人物对白继续由 SadTalker 按完整音频驱动，林小雨与陈默各自只使用一个固定身份锚点。
- 发布质量门通过，`issues` 为空；镜头 11 未检测到超过 1.5 秒的冻结。
- 最终交付：`final/perfect_lover_v3_hybrid_captioned.mp4`，1080×1920、30 fps、175.507 秒。

## V4 第一版全画幅构图修复

V3 的头像小窗和模糊背景方案被废弃。V4 回到第一版的整幅场景构图，只对异常片段做受限重试和安全区间裁切。

- 所有镜头保持第一版完整的 9:16 构图，不使用头像框、清晰小窗或模糊填充。
- 每个镜头只播放一次，再用运动插帧均匀延展至对白长度，不循环回首帧。
- 镜头 02 删除出现额外手的前段，只保留连续抽帧均为两只手和一只杯子的完整画幅区间。
- 镜头 10 删除亮度上升的尾段，只保留亮度稳定的 0–2.8 秒。
- 镜头 11 使用新生成的连续微动作完整画幅候选，亮度均值约 113.7–116.9。
- 完整成片未检测到超过 1 秒的冻结；发布质量门通过，`issues` 为空。
- 最终交付：`final/perfect_lover_v4_fullframe_captioned.mp4`，1080×1920、30 fps、175.5 秒。
