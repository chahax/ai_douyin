...
当前项目主线已经切到“动漫数字人主讲”：关键词/文章/文案 -> 脚本 -> 分段配音 -> Sonic 角色视频层 -> 动漫背景 -> 字幕避让 -> FFmpeg 合成 -> 抖音发布。管理后台和 `AutoPublishRequest.video_mode` 当前默认都是 `presenter_anime`。
...
| 小说推广视频平台支线 | 设计中 | `docs/NOVEL_PROMOTION_VIDEO_PLATFORM_DESIGN.md` | 规划番茄推广平台任务申请、小说前 10 章提炼、推广视频生成和绑定；尚未实现 |
