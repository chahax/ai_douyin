...
关键词输入
    ↓
AI 生成讲解脚本（RAG 知识库 + 大语言模型）
    ↓
AI 语音合成配音（默认 Edge-TTS，可选 GPT-SoVITS）
    ↓
BGM 背景音乐混音
    ↓
视频片段自动循环合成
...
当前这条链路已接入单人口播模板视频模式，并可通过浏览器自动化上传到抖音。

### 2. 知识库辅助创作

内置古典典籍知识库（论语、孟子等），生成脚本时可引用古文智慧为内容增添深度。

```
用户输入关键词 → RAG 语义检索 → 匹配相关知识片段 → AI 结合知识生成脚本
```
...
| 路线 | 状态 | 说明 |
|---|---|---|
| 单人口播模板视频 | 稳定可用 | `auto-publish` 默认路线 |
| 动漫数字人主讲 | 半自动可用 | Edge-TTS + Sonic 角色层 + ComfyUI 分段背景已验证 |
| 双角色/FramePack | 半自动可用 | 已有样片和合成代码，未接入默认一键发布 |
...
配置关键词回复规则后，AI 自动扫描视频下的评论，对符合规则的评论进行智能回复。

```
评论进来
    ↓
匹配关键词规则（RAG 检索找到最相关的规则）
    ↓
固定回复 / 大模型生成回复
    ↓
自动发布至评论区
