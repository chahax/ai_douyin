...
3. 给 `auto-publish` 增加模式参数，例如 `single`、`dual`、`framepack`。
4. 等生成链路稳定后，再推进调度、API、队列和部署。
