...
- 关键词输入，PDF/TXT 书籍解析 + RAG 知识检索，AI 生成爆款逐字稿（Hook + Body + 行动建议）。
- Edge-TTS + GPT-SoVITS 多音色 TTS 配音，支持 BGM 混音。
- MoviePy 动态字幕、图片转短视频（9:16 1080×1920 MP4）。
- Selenium 模拟浏览器登录抖音，自动上传、标题/标签填写、发布。
- 同步抖音视频、抓取评论、AI 生成回复话术（意图分类 + 安全过滤）、自动发送评论/私信。
- 运行 Streamlit 管理后台（多角色分级权限）。
